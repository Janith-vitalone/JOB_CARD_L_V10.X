                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <form action="{{ route('finishing.update', $finishing->id) }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" id="name" value="{{ $finishing->name }}" class="form-control @error('name') is-invalid @enderror" required>
                                        @error('name')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="rate" id="rate" value="{{ $finishing->rate }}" class="form-control @error('rate') is-invalid @enderror" required>
                                        @error('rate')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
...
@section('js')
    <script>
        $(document).ready(function() {
            $('#job_type').val('{{ $finishing->job_type }}');
            getFormats('{{ $finishing->job_type }}');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (!response.error) {
                            ...
                            $('#format').val('{{ $finishing->format_id }}')
