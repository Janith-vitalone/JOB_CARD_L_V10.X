                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12">
                            <form action="#" method="POST" id="invoice_details">
                                ...
                                <div class="form-group">
                                    ...
                                    <input name="client" id="client" class="form-control" value="{{ $client->name ?? "-" }} | {{ $client->contact_person ?? "-" }} | {{ $client->phone ?? "-" }}" disabled required>
                                ...
                                <input type="text" value="{{ $invoice->id }}" id="invoiceId" hidden>
                                <div class="form-group">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-custom spacing8">
                            ...
                            <tbody id="products">
                                ...
                                <tr>
                                    <td>{{ $key }}</td>
                                    <td>{{ $invoice_product->products->stockProductCategory->name ?? "-" }}</td>
                                    <td>{{ $invoice_product->products->name ?? "-" }}</td>
                                    <td>{{ $invoice_product->description }}</td>
                                    <td>{{ $invoice_product->qty }}</td>
                                    <td>{{ $invoice_product->unit_price }}</td>
                                    <td>{{ ($invoice_product->unit_price * $invoice_product->qty) }}</td>
                                    <td hidden><input type="text" class="product_id" value="{{ $invoice_product->products->id }}"></td>
                                    <td>
                                        <button class="btn btn-xs btn-danger del-btn">
            ...
            <div class="card">
                ...
                <div id="product-form" class="body">
                    <!-- <form method="POST" action="{{ route('stockin.store') }}" id="frm" enctype="multipart/form-data"> -->
                        ...
                        <div class="row clearfix">
                            <div class="col-lg-3 col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="subtotal" id="subtotal" class="form-control" value="{{ $invoice->sub_total }}" disabled required>
                                </div>
                            </div>
                        </div>
                        <div class="row clearfix">
                        ...
                        <div class="row clearfix">
                            <div class="col-lg-3 col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="total" id="total" class="form-control" value="{{ $invoice->grand_total }}" disabled required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row clearfix">
                            <div class="col-lg-3 col-md-12">
                                <h6>Payment status :  <span style="color: rgb(240, 224, 0)">{{ $invoice->payment_status }}</span></h6>
                                <h6>Paid Amount :  <span style="color: rgb(240, 224, 0)">{{ number_format($invoice->payments->sum('paid_amount'), 2) }}</span></h6>
                            </div>
                        </div>
                        @else
                        <div class="row clearfix">
                            <div class="col-lg-3 col-md-12">
                                <h6>Payment status :  <span style="color: green">Paid</span></h6>
                                <h6>Paid Amount :  <span style="color: green">{{ number_format($invoice->payments->sum('paid_amount'), 2) }}</span></h6>
                            </div>
                        </div>
                        @endif
                        <div class="row clearfix">
                            <div class="col-lg-3 col-md-12">
                                <div class="form-group">
                                    <button type="button" id="create" class="btn btn-primary" id="addnew_product">Update</button>
                                </div>
                            </div>
                        </div>
                    <!-- </form> -->
    ...
    <script>
        ...
        jQuery(document).ready(function ($){
            ...
            $("#addRow").click(function(){
                ...
                var currentSubtotal = Number($("#subtotal").val());
                var currentDiscount = Number($("#discount").val());
                currentSubtotal = currentSubtotal + total;
                var currentDiscount = (currentSubtotal * currentDiscount)/100;
                var currentTotal = currentSubtotal - currentDiscount;
                ...
                // $("#"+ i).click(function(e) {
                //     e.preventDefault();
                //     currentSubtotal = currentSubtotal - Number($(this).closest('tr').find('td:eq(5)').text());
                //     currentSubtotal = currentSubtotal - Number($(this).closest('tr').find('td:eq(5)').text());
                //     $("#total").val(netValue);
            ...
            $("#products").on('click','.del-btn',function(){
                var delRowTotal = Number($(this).closest('tr').find('td:eq(5)').text());
                var oldSubTotal = $("#subtotal").val();
                var oldDiscount = $("#discount").val();
                var oldTotal = $("#total").val();

                var newSubtotal = oldSubTotal - delRowTotal;
                var newDiscount = (newSubtotal * oldDiscount)/100;
            ...
            $("#create").on('click', function(e) {
                ...
                $.ajax({
                    ...
                    url: "{{ config('app.url') }}/invoice/quick/update/"+invoiceId,
                    type: "POST",
